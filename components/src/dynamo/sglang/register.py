...
def _get_bootstrap_info_for_config(
    ...
    try:
        ...
        if inner_tm.server_args.dist_init_addr:
            ...
            logging.info(f"Bootstrap host from dist_init_addr: {bootstrap_host}")
        else:
            bootstrap_host = get_local_ip_auto()
            logging.info(f"Using auto-detected local IP: {bootstrap_host}")

        # Use NetworkAddress for IPv6 bracket wrapping
